// Code your solution here!

function printString(myString) {
  console.log(myString[0]);

  if (myString.length > 1) {
    let mySubString = myString.substring(1, myString.length);
    printString(mySubString);
  } else {
    return true;
  }
}

function reverseString(myString) {

  if(myString.length < 2) {
    return myString
  }

  return myString.charAt(myString.length - 1) + reverseString(myString.substring(0, myString.length - 1))

}

function isPalindrome(myString) {
  if(myString.length < 2) {
    return true
  }

  if(myString.charAt(0) === myString.charAt(myString.length-1)) {
    return isPalindrome(myString.substring(1, myString.length-1))
  } else {
    return false
  }

}

function addUpTo(arr, idx) {
    if(idx === 0) {
      return arr[0]
    } else {
      return arr[idx] + addUpTo(arr, idx - 1)
    }

}

function maxOf(arr) {
  if(arr.length < 2) {
    return arr[0]
  }

  let decimatedArr = arr;

  if(arr[0] < arr[1]) {
    decimatedArr.splice(0,1)
    return maxOf(decimatedArr)
  } else {
    decimatedArr.splice(1,1)
    return maxOf(decimatedArr)
  }

}

function includesNumber(arr, num) {
  if(arr.length < 2) {
    return arr[0] === num
  }

  if(arr[0] === num) {
    return true
  }

  arr.splice(0,1);
  return includesNumber(arr, num)

}
